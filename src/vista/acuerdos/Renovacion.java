...
public class Renovacion extends javax.swing.JFrame {
    ...
    public static boolean validarNombre(String nombre){ //Metodo que valida el nombre
      ...
      public void cortarCBP( javax.swing.JComboBox combo ){
        ...
        part1 = parts[0].replaceAll("Renovacion#", " ");//Nombre del cliente
        part2 = parts[1].replaceAll("Contrato#", " ");
        part3 = parts[2];//nombre de la varieble
        part4 = parts[3];
}
    
    public void llenarRenovacion(){
        cbRenovacion.removeAllItems();
        cbRenovacionEli.removeAllItems();
        cbRenovacion.addItem(" ");
        cbRenovacionEli.addItem(" ");
        try {
            Statement stmt = controllerLogin.conexion.createStatement();
            ResultSet rs = stmt.executeQuery( "select 'Renovacion#' || re.id_secuencia || ' / ' || 'Contrato#' || co.id || ' / ' ||   po.nombre || ' / ' ||  ci.denominacion_comercial as texto " +
            "from aja_renovacion re ,aja_contrato co, aja_cliente ci, aja_productor po " +
            "where re.id_cliente = ci.id and re.id_productor = po.id and re.id_contrato=co.id"
                    + " order by  re.id_secuencia asc" );
            while ( rs.next() ) {
            String registro = rs.getString(1);
            cbRenovacion.addItem(registro);
            cbRenovacionEli.addItem(registro);
            }
    ...
    public void limpiar(){
        ...
        cbRenovacion.setSelectedIndex(0);
        cbRenovacionEli.setSelectedIndex(0);
    }
    
    public void inserts(){
        
       if(dcFecha.getDate()==null || spTotal.getValue()==null){
            JOptionPane.showMessageDialog(null, "No hay ninguna fecha registrada");
        } else{
            try {
            
           Statement stmt = controllerLogin.conexion.createStatement();
           String sql = "INSERT INTO aja_renovacion (id_cliente, id_productor, id_contrato, fecha, total) values ("
    ...
    public void delete(){
        ...
        try{
                Statement stmt = controllerLogin.conexion.createStatement();
                String sql = "DELETE FROM aja_renovacion where  id_secuencia = " + id;

                stmt.executeUpdate(sql);
                controllerLogin.conexion.commit();
                JOptionPane.showMessageDialog(null,"Se ha borrado la renovacion exitosamente");
                llenarRenovacion();
                limpiar();
    ...
    public void update(){
       if(dcFecha.getDate()==null || spTotal.getValue()==null){
            ...
            try{
                Statement stmt = controllerLogin.conexion.createStatement();
              String sql = "update aja_renovacion set (id_cliente, id_productor, id_contrato, fecha, total) = ("
                  + "(select id from aja_cliente where denominacion_comercial='"+ part4.trim() + "'),"
                  + "(select id from aja_productor where nombre='"+ part3.trim() + "'),"
                  + "(select id from aja_contrato where id="+ part2.trim() + "),'"
                  + format.format(dcFecha.getDate()) +"' ,"
                  +  spTotal.getValue() +") "
                      + " where id_secuencia = "+ part1.trim() + " ";
    ...
    private void initComponents() {
        ...
        cbRenovacion = new javax.swing.JComboBox<>();
        btnModificar = new javax.swing.JButton();
        jPanel2 = new javax.swing.JPanel();
        btnDelete = new javax.swing.JButton();
        cbRenovacionEli = new javax.swing.JComboBox<>();
        ...
        jVariedadPais1.setText("Renovacion");
        jPanel3.add(jVariedadPais1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 10, -1, -1));

        cbRenovacion.setFont(new java.awt.Font("Segoe UI", 0, 11)); // NOI18N
        cbRenovacion.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { " " }));
        cbRenovacion.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cbRenovacionActionPerformed(evt);
            }
        });
        jPanel3.add(cbRenovacion, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 40, 400, 25));
        ...
        cbRenovacionEli.setFont(new java.awt.Font("Segoe UI", 0, 11)); // NOI18N
        cbRenovacionEli.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { " " }));
        cbRenovacionEli.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cbRenovacionEliActionPerformed(evt);
            }
        });
        jPanel2.add(cbRenovacionEli, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 40, 420, 25));

        jLabel8.setBackground(new java.awt.Color(0, 0, 0));
        jLabel8.setFont(new java.awt.Font("Dialog", 1, 18)); // NOI18N
        jLabel8.setForeground(new java.awt.Color(255, 255, 255));
        jLabel8.setText("Renovacion");
        ...
        jNombreVar1.setText("Total precio nuevo");
        jPanel1.add(jNombreVar1, new org.netbeans.lib.awtextra.AbsoluteConstraints(340, 150, 170, 24));
        jPanel1.add(spTotal, new org.netbeans.lib.awtextra.AbsoluteConstraints(340, 180, 180, 30));
        ...
        jNombreVar5.setText("Fecha renovacion:");
    ...
    private void cbRenovacionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbRenovacionActionPerformed
        if ( cbRenovacion.getItemCount() > 1 ){
            cbRenovacionEli.setEnabled(false);
            if (!cbRenovacion.getSelectedItem().toString().equals(" ")){
               
                cortarCBP(cbRenovacion);
                ...
                try {
                   ...
                   ResultSet rs = stmt.executeQuery("select re.id_secuencia, re.fecha, re.total "
                            + "from aja_renovacion re, aja_cliente ci, aja_productor pr, aja_contrato co"
                            + " where re.id_secuencia = '"+ part1.trim() +"' "
                           + " and re.id_cliente = (select ci.id from aja_cliente ci where ci.denominacion_comercial = '"+ part4.trim() +"') and ci.id=re.id_cliente"
                            + " and re.id_productor = (select pr.id from aja_productor pr where pr.nombre = '"+ part3.trim() +"') and pr.id = re.id_productor"
                            + " and re.id_contrato = '"+ part2.trim() +"' and co.id = re.id_contrato");
                ...
                btnModificar.setEnabled(false);
                btnInsertar.setEnabled(false);
                cbRenovacionEli.setEnabled(true);
                limpiar();
            }
        }    
    }//GEN-LAST:event_cbRenovacionActionPerformed

    private void cbRenovacionEliActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbRenovacionEliActionPerformed
        if ( cbRenovacionEli.getItemCount() > 1 ){
                cbRenovacion.setEnabled(false);
                if (!cbRenovacionEli.getSelectedItem().toString().equals(" ")){
                    
                cortarCBP(cbRenovacionEli);
                btnModificar.setEnabled(false);
                btnInsertar.setEnabled(false);
                ...
                cbRenovacion.setSelectedIndex(0);
                btnDelete.setEnabled(false);
                cbRenovacion.setEnabled(true);
            }
        }
        
    }//GEN-LAST:event_cbRenovacionEliActionPerformed
    ...
    private javax.swing.JComboBox<String> cbRenovacion;
    private javax.swing.JComboBox<String> cbRenovacionEli;
