...
public class ConvenioTrabajo extends javax.swing.JFrame {
    ...
    public static boolean validarNombre(String nombre){ //Metodo que valida el nombre
      ...
      public void cortarCBP( javax.swing.JComboBox combo ){
        ...
        part3 = parts[2].replaceAll("Productor: ", " ");
}
    
    public void llenarConvenioT(){
        cbConvenioT.removeAllItems();
        cbConvenioTEli.removeAllItems();
        cbConvenioT.addItem(" ");
        cbConvenioTEli.addItem(" ");
        try {
            Statement stmt = controllerLogin.conexion.createStatement();
            ResultSet rs = stmt.executeQuery( "select 'Año: ' || co.fecha_firma || ' / ' || 'Proveedor: ' || po.nombre || ' / ' || 'Productor: ' || pr.nombre as texto " +
            "from aja_convenio_de_trabajo co, aja_productor pr, aja_proveedor po " +
            "where co.id_productor= pr.id and co.id_proveedor = po.id"
                    + " order by  co.fecha_firma asc" );
            while ( rs.next() ) {
            String registro = rs.getString(1);
            cbConvenioT.addItem(registro);
            cbConvenioTEli.addItem(registro);
            }
    ...
    public void limpiar(){
        ...
        cbConvenioT.setSelectedIndex(0);
        cbConvenioTEli.setSelectedIndex(0);
    }
    ...
    public void inserts(){
       ...
       if(dcFecha.getDate()==null){
           ...
           Statement stmt = controllerLogin.conexion.createStatement();
           String sql = "INSERT INTO aja_convenio_de_trabajo (id_proveedor, id_productor, fecha_firma, beneficios, vigencia) values ("
           ...
           if(format.format(dcFecha.getDate()).equals(format.format(fecha))){
               JOptionPane.showMessageDialog(null, " la fecha no puede ser la misma");
                    ...
                    JOptionPane.showMessageDialog(null,"Se ha registrado el convenio de trabajo exitosamente");
                    llenarConvenioT();
                    limpiar();
           }
    ...
    public void delete(){
        ...
        try{
                Statement stmt = controllerLogin.conexion.createStatement();
                String sql = "DELETE FROM aja_conveniode_trabajo where  fecha_firma = '"+ id +"' " ;
    ...
    public void update(){
       if(dcFecha.getDate()==null){
            ...
            try{
                Statement stmt = controllerLogin.conexion.createStatement();
              String sql = "update aja_convenio_de_trabajo set (beneficios, vigencia) = ('"
                  +  txtBeneficios.getText() +"', '"
                 +  cbVigencia.getSelectedItem().toString() +"')"
    ...
    private void initComponents() {
        ...
        cbConvenioT = new javax.swing.JComboBox<>();
        btnModificar = new javax.swing.JButton();
        jPanel2 = new javax.swing.JPanel();
        btnDelete = new javax.swing.JButton();
        cbConvenioTEli = new javax.swing.JComboBox<>();
        ...
        jVariedadPais1.setText("Convenio Trabajo");
        jPanel3.add(jVariedadPais1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 10, -1, -1));

        cbConvenioT.setFont(new java.awt.Font("Segoe UI", 0, 11)); // NOI18N
        cbConvenioT.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { " " }));
        cbConvenioT.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cbConvenioTActionPerformed(evt);
            }
        });
        jPanel3.add(cbConvenioT, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 40, 400, 25));
        ...
        cbConvenioTEli.setFont(new java.awt.Font("Segoe UI", 0, 11)); // NOI18N
        cbConvenioTEli.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { " " }));
        cbConvenioTEli.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cbConvenioTEliActionPerformed(evt);
            }
        });
        jPanel2.add(cbConvenioTEli, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 40, 420, 25));

        jLabel8.setBackground(new java.awt.Color(0, 0, 0));
        jLabel8.setFont(new java.awt.Font("Dialog", 1, 18)); // NOI18N
        jLabel8.setForeground(new java.awt.Color(255, 255, 255));
        jLabel8.setText("Convenio Trabajo");
    ...
    private void cbConvenioTActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbConvenioTActionPerformed
        if ( cbConvenioT.getItemCount() > 1 ){
            cbConvenioTEli.setEnabled(false);
            if (!cbConvenioT.getSelectedItem().toString().equals(" ")){
               
                cortarCBP(cbConvenioT);
                ...
                try {
                   ...
                   ResultSet rs = stmt.executeQuery("select co.fecha_firma, co.beneficios, co.vigencia "
                            + "from aja_convenio_de_trabajo co, aja_productor po, aja_proveedor pr"
                            + " where co.fecha_firma = '"+ part1.trim() +"' "
                           + " and co.id_productor = (select po.id from aja_productor po where po.nombre= '"+ part3.trim() +"') and po.id=co.id_productor"
                ...
                btnModificar.setEnabled(false);
                btnInsertar.setEnabled(false);
                cbConvenioTEli.setEnabled(true);
                limpiar();
            }
        }    
    }//GEN-LAST:event_cbConvenioTActionPerformed

    private void cbConvenioTEliActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbConvenioTEliActionPerformed
        if ( cbConvenioTEli.getItemCount() > 1 ){
                cbConvenioT.setEnabled(false);
                if (!cbConvenioTEli.getSelectedItem().toString().equals(" ")){
                    
                cortarCBP(cbConvenioTEli);
                btnModificar.setEnabled(false);
                btnInsertar.setEnabled(false);
                ...
                cbConvenioT.setSelectedIndex(0);
                btnDelete.setEnabled(false);
                cbConvenioT.setEnabled(true);
            }
        }
        
    }//GEN-LAST:event_cbConvenioTEliActionPerformed
    ...
    private javax.swing.JComboBox<String> cbConvenioT;
    private javax.swing.JComboBox<String> cbConvenioTEli;
